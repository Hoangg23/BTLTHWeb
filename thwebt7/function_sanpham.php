...
class sanpham extends ketnoi{
    ...
    function themsanpham(){
        $sql = "INSERT INTO `sanpham1`(`masp`, `nhom_id`, `tensp`, `mota`, `dongiacu`, `dongiamoi`,  `enable`, `ghichu`, `diemnoibat`,  `xuatxu`, `ngaydang`, `kichthuoc`, `nguoidang`, `loaiao`, `soluong`)
    VALUES ('".$this->masanpham."','".$this->nhomid."','".$this->tensanpham."','".$this->mota."','".$this->dongiacu."','".$this->dongiamoi."', '".$this->enable."','".$this->ghichu."','".$this->diemnoibat."', '".$this->xuatxu."', '".$this->ngaydang."','".$this->kichthuoc."','".$this->nguoidang."', '".$this->loaiao."', '".$this->soluong."')";
    $result = mysqli_query($this->conn, $sql);
    ...
    function updatesanpham(){
        ...
        if (move_uploaded_file($_FILES["img3"]["tmp_name"], $targer_file3)) {
            ...
            if (!mysqli_query($this->conn, $sql)) {
                echo "Error updating img3: " . mysqli_error($this->conn);
            }
        }
        ...
        if (move_uploaded_file($_FILES["img4"]["tmp_name"], $targer_file4)) {
            $sql = "UPDATE `sanpham1` SET `img4`='$filename4' WHERE masp = '".$this->masanpham."'";
            if (!mysqli_query($this->conn, $sql)) {
                echo "Error updating img4: " . mysqli_error($this->conn);
            }
        }
